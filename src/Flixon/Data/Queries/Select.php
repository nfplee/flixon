...
namespace Flixon\Data\Queries;

use ArrayIterator;
use Envms\FluentPDO\Queries\Select as SelectBase;
...
class Select extends SelectBase {
	...
	public function asEntity(?string $class = null): Select {
		$this->class = $class;
	...
	public function fetch(?string $column = null, int $cursorOrientation = \PDO::FETCH_ORI_NEXT) {
		...
		$row = parent::fetch($column, $cursorOrientation);
	...
	public function fetchAll(mixed $index = '', mixed $selectOnly = ''): Enumerable {
		// Call the parent method.
	...
	public function with(string $name, ?string $table = null, ?string $foreignKey = null): Select {
	...
	private function withOne(string $name, ?string $table = null, ?string $foreignKey = null): Select {
		...
		$fromAlias = str_contains($name, '.') ? str_replace('.', '_', substr($name, 0, strrpos($name, '.'))) : $this->fromAlias;
